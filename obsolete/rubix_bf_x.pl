...
expand_next(Tail, Conf, Trace, [Act | Acts], Next) :-
    ...
    (
        visited(ConfNext), !, expand_next(Tail, Conf, Trace, Acts, Next)
        ;
        assert(visited(ConfNext)),
        rotate(ConfNext, rz1, Z1), assert(visited(Z1)),
        rotate(ConfNext, rz2, Z2), assert(visited(Z2)),
        rotate(ConfNext, rz3, Z3), assert(visited(Z3)),
        rotate(ConfNext, rx1, X1), assert(visited(X1)),
        rotate(ConfNext, rx2, X2), assert(visited(X2)),
        rotate(ConfNext, rx3, X3), assert(visited(X3)),
        rotate(ConfNext, ry1, Y1), assert(visited(Y1)),
        rotate(ConfNext, ry2, Y2), assert(visited(Y2)),
        rotate(ConfNext, ry3, Y3), assert(visited(Y3)),
