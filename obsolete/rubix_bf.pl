...
solve_cur_first([ Conf/Trace | Tail], Next, Visited) :-
    ...
    ;
    write(Trace), nl,
    expand_next(Tail, Conf, Trace, [f, f_, r, r_, b, b_, l, l_, u, u_, d, d_, m, m_, e, e_, s, s_], Next, Visited).

expand_next(Tail, _, _, [], Next, Visited) :- solve_cur_first(Tail, Next, Visited).
expand_next(Tail, Conf, Trace, [Act | Acts], Next, Visited) :-
    move(Conf, Act, ConfNext),
    (
        visited(ConfNext, Visited), !, expand_next(Tail, Conf, Trace, Acts, Next, Visited)
        ;
        expand_next(Tail, Conf, Trace, Acts, [ConfNext/[Act|Trace] | Next], [ConfNext | Visited])
    ).

solve(X) :- solve_cur_first([ X/[] ], [], [X]).

visited(X, [H | T]) :- X == H; rotate(X, _, H); visited(X, T).
